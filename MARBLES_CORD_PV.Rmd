...
pd <- readRDS("/dcl01/NDEpi/data/Projects/InProgress/jdou/MARBLES_EPIC_blood/QC/qc_04_snp.rds")
pd.cov$Plate <- pd[match(pd.cov$COI_ID, pd$COI_ID), 'Sample_Plate']
```
...
for.plot$batch <- as.factor(pd.mod$Plate)
...
for(PC in colnames(heatmap.pc)){
  ...
  heatmap.pc["batch",PC] <- summary(aov(for.plot[,PC] ~ for.plot$batch))[[1]][1,5]
}
...
### model
```{r}
mod <- model.matrix(~factor(pd.mod$PVmo1) + pd.mod$sex + pd.mod$mom_age + pd.mod$gest_age + pd.mod$mom_edu_bn + pd.mod$pc1 + pd.mod$pc2 + pd.mod$pc3 + pd.mod$pc4 + pd.mod$pc5 + pd.mod$Gran + pd.mod$nRBC + pd.mod$Plate)
